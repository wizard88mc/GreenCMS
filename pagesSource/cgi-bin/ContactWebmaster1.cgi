...
sub printFormMessage() {
	...
	my $content = <<CONTENT;
	$message
	<form method="post" action="ContactWebmaster1.cgi">
	<fieldset>
	...
	<label for="mailcheck" class="aiutoxVisualizzazione">Protezione dai bot, non riempire questo campo: </label>
	<textarea id="mailcheck" class="aiutoxVisualizzazione" name="mailcheck" cols="20" rows="5">$userFormInput{'mailcheck'}</textarea>
	</fieldset>
	<fieldset>
...
sub checkFields() {
	...
	#se questo campo non è vuoto si tratta di un bot
	if ($userFormInput{'mailcheck'} ne "") {
		die "mail da bot " . $userFormInput{'email'}; 
	}
...
if ($userFormInput{'submit'} eq "Spedisci") {
	...
	$userFormInput{'mailcheck'} = $page->param('mailcheck');
