...
<div id="contents">
	...
	<form method="post" action="NewSeminary.cgi">
		...
		<p><strong>Abstract: </strong>$userFormInput{'abstract'}</p>
...
<div id="contents">
	...
	<input type="text" name="date" id="date" value="$userFormInput{'date'}" /><br />
...
sub checkInputs() {
	...
	if ($userFormInput{'date'} =~ m/^\d{2}\/\d{2}\/\d{4}$/) {
		my $day = substr($userFormInput{'date'}, 0, 2);
		my $month = substr($userFormInput{'date'}, 3, 2);
		my $year = substr($userFormInput{'date'}, 6);
		if ($month == "02" and $day > "28") {
			$errors .= "Giorno di seminario errato<br />";
		}
		...
		$errors .= "Formato data non corretto (gg/mm/aaaa)<br />";
	}
