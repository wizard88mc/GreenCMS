...
window.onload = function() {
	...
	xmlhttp.onreadystatechange = function() {
		...
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			activeNews = xmlhttp.responseXML.documentElement.getElementsByTagName("ActiveNews");
			...
			for (i = 0; i < activeNews.length && i < 3; i++) {  //stampo al massimo 3 news
				...
				if (activeNews[i].getElementsByTagName("Type")[0].firstChild.nodeValue == "LM") {
			...
			if (count == 0) {
				...
				ddNoNews.innerHTML = "Non ci sono news specifiche per la Laurea";
